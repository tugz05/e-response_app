...
class TwilioVoicePlugin : FlutterPlugin, MethodCallHandler, EventChannel.StreamHandler,
    ...
    private val callSid: String?
        get() = InAppOutgoingCalls.getActiveSid() ?: TVConnectionService.getActiveCallHandle()
...
//                callOutgoing = true
    ...
    private fun sendDigits(digits: String): Boolean {
        if (InAppOutgoingCalls.sendDigits(digits)) {
            return true
        }
        context?.let { ctx ->
...
//        return tm.isOnCall(ctx)
        return InAppOutgoingCalls.isActive() || TVConnectionService.hasActiveCalls()
    }

    private fun toggleSpeaker(ctx: Context, speakerIsOn: Boolean) {
        if (InAppOutgoingCalls.setSpeaker(ctx, speakerIsOn)) {
    ...
    private fun toggleMute(ctx: Context, mute: Boolean) {
        if (InAppOutgoingCalls.setMute(mute)) {
            isMuted = mute
    ...
    private fun toggleBluetooth(ctx: Context, bluetoothOn: Boolean) {
        if (InAppOutgoingCalls.setBluetooth(ctx, bluetoothOn)) {
    ...
    private fun toggleHold(ctx: Context, shouldHold: Boolean) {
        if (InAppOutgoingCalls.setHold(shouldHold)) {
    ...
    /**
     * Places an outbound call in-app via [Voice.connect] (see [InAppOutgoingCalls]) — no
     * [android.telecom.TelecomManager.placeCall], so the system Phone app is not opened.
    ...
    private fun placeCall(
        ...
        return InAppOutgoingCalls.start(ctx, accessToken, from, to, callParams)
    }
