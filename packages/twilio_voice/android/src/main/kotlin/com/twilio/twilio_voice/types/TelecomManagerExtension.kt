...
//            return
        ...
        // Self-managed (Android 10+): VoIP UI stays in-app; avoids default Phone/Dialer handoff.
        // Older APIs keep the stock Twilio plugin PhoneAccount (still uses system Telecom integration).
        val phoneAccount =
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                PhoneAccount.builder(phoneAccountHandle, label)
    ...
    fun TelecomManager.hasCallCapableAccount(ctx: Context, name: String): Boolean {
        ...
        // Self-managed accounts (Android 10+) are not always listed in callCapablePhoneAccounts.
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            try {
                @Suppress("UNCHECKED_CAST")
                val method = TelecomManager::class.java.getMethod("getSelfManagedPhoneAccounts")
                val raw = method.invoke(this) as? Iterable<*> ?: emptyList<Any>()
                for (p in raw) {
                    if (p == ctx.packageName) return true
                }
            } catch (_: Throwable) {
                // ignore
            }
